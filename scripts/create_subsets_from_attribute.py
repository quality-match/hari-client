...
from hari_client import Config, HARIClient
from hari_client.models import models
from hari_client.models.models import QueryParameter, QueryList, SubsetType


def create_subsets_for_attribute(dataset_id: str, attribute_id: str, prefix: str) -> None:
    """
    ...
    # 1. Initialize the HARI client using the configuration from the .env file.
    ...
    labels: List[str] = list(set([
        label
        for attribute in attributes if attribute.frequency is not None
    ...
    visconfigs: List[models.VisualisationConfiguration] = hari.get_vis_configs(dataset_id)
    if not visconfigs:
...
if __name__ == '__main__':
    ...
    create_subsets_for_attribute(dataset_id, attribute_id, prefix)
