...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "Videos can not contain media objects.",
        ),
    ],
)
...
def test_hari_uploader_skips_uploaded_entities(
    ...
    assert results.media_objects.results[0].status == models.ResponseStatesEnum.SUCCESS
    assert (
        results.media_objects.results[1].status
        == models.ResponseStatesEnum.ALREADY_EXISTS
    )
...
def test_hari_uploader_uses_referenced_media(
    create_configurable_mock_uploader_successful_single_batch_state_aware,
):
    # Arrange
    (
        ...
        dataset_id=uuid.UUID(int=0),
        medias_cnt=0,  # no media uploaded
        media_objects_cnt=1,  # 1 to be uploaded
        attributes_cnt=2,  # 2 to be uploaded
    )
    ...
    # create referenced media
    referenced_media = hari_uploader.ReferencedMedia(
        back_reference="media_ref_1", media_type=models.MediaType.IMAGE
    )

    # create objects to be uploaded
    new_media_object = create_test_media_object_2d(back_reference="media_obj_ref_2")
    referenced_media.add_media_object(new_media_object)

    # create attributes to be uploaded
    new_media_attribute = create_test_attribute(
        id=uuid.UUID(int=1), name="media_attr_1"
    )
    referenced_media.add_attribute(new_media_attribute)
    new_media_object_attribute = create_test_attribute(
        id=uuid.UUID(int=3), name="media_obj_2"
    )
    new_media_object.add_attribute(new_media_object_attribute)

    # Act
    uploader.add_media(referenced_media)
    results = uploader.upload()

    # Assert
    assert create_medias_spy.call_count == 0  # no media uploaded
    assert len(results.medias.results) == 1  # 1 already existing media
    assert results.medias.results[0].status == models.ResponseStatesEnum.ALREADY_EXISTS

    assert create_media_objects_spy.call_count == 1  # media object was uploaded
    assert len(results.media_objects.results) == 1
    assert results.media_objects.results[0].status == models.ResponseStatesEnum.SUCCESS

    assert create_attributes_spy.call_count == 1  # attributes were uploaded
    assert len(results.attributes.results) == 2
    for result in results.attributes.results:
        assert result.status == models.ResponseStatesEnum.SUCCESS


def test_hari_uploader_uses_referenced_media_object(
    create_configurable_mock_uploader_successful_single_batch_state_aware,
):
    # Arrange
    (
    ...
    # create referenced media object
    referenced_media_object = hari_uploader.ReferencedMediaObject(
        back_reference="media_obj_ref_1"
    )
    referenced_media.add_media_object(referenced_media_object)

    # create object attribute to be uploaded
    new_media_object_attribute = create_test_attribute(
        id=uuid.UUID(int=3), name="media_obj_2"
    )
    referenced_media_object.add_attribute(new_media_object_attribute)
    ...
    assert create_media_objects_spy.call_count == 0  # no media objects uploaded
    assert len(results.media_objects.results) == 1  # 1 already existing media object
    assert (
        results.media_objects.results[0].status
        == models.ResponseStatesEnum.ALREADY_EXISTS
    )

    assert create_attributes_spy.call_count == 1  # attributes were uploaded
    assert len(results.attributes.results) == 1
    assert results.attributes.results[0].status == models.ResponseStatesEnum.SUCCESS


def test_hari_uploader_uses_referenced_models_mixed(
    create_configurable_mock_uploader_successful_single_batch_state_aware,
):
    # This test checks that the uploader correctly handles a mix of new and referenced media and media objects.
    ...
    (
        ...
        medias_cnt=1,  # 1 new media
        media_objects_cnt=1,  # 1 new media object
        attributes_cnt=4,  # 1 new for each entity
    )
    ...
    # create new
    new_media = create_test_media(back_reference="media_ref_3")
    new_media_object = create_test_media_object_2d(back_reference="media_obj_ref_3")
    ...
    # add 1 new and 1 referenced media object to referenced media
    referenced_media.add_media_object(referenced_media_object)
    referenced_media.add_media_object(new_media_object)
    ...
    # Act
    uploader.add_media(referenced_media, new_media)
    results = uploader.upload()

    # Assert
    assert create_medias_spy.call_count == 1
    media_calls = create_medias_spy.call_args_list
    assert len(media_calls[0].kwargs["medias"]) == 1  # only new was uploaded
    assert (
        len(results.medias.results) == 2
    )  # but 2 results as we also include already existing media
    assert results.medias.results[0].status == models.ResponseStatesEnum.SUCCESS
...
def test_hari_uploader_referenced_media_invalid_reference(
    create_configurable_mock_uploader_successful_single_batch_state_aware,
):
    # Arrange
    (
    ...
    # create referenced media
    referenced_media = hari_uploader.ReferencedMedia(
        back_reference="invalid", media_type=models.MediaType.IMAGE
    )
    ...
    assert (
        str(exc_info.value)
        == "Reference model's back reference is not found on the server: 'invalid'"
    )


def test_hari_uploader_referenced_media_object_invalid_reference(
    create_configurable_mock_uploader_successful_single_batch_state_aware,
):
    # Arrange
    (
    ...
    # create referenced media (valid reference)
    referenced_media = hari_uploader.ReferencedMedia(
        back_reference="media_ref_1", media_type=models.MediaType.IMAGE
    )

    # create referenced media object (invalid reference)
    referenced_media_object = hari_uploader.ReferencedMediaObject(
        back_reference="invalid"
    )
    referenced_media.add_media_object(referenced_media_object)
