...
from hari_client.models.models import BulkMediaObjectCreate
...
# all_media_objects=hari.get_media_objects(dataset_id=dataset_id)
objects = []
for media in all_media[:500]:
    for i in range(4):
        obj = BulkMediaObjectCreate(
            bulk_operation_annotatable_id=str(uuid.uuid4()),
            media_id=media.id,
...
# cop=copy.deepcopy(objects)
# extra=cop[:500]
# for e in extra:
#     e.bulk_operation_annotatable_id=str(uuid.uuid4())
# objects.extend(extra)

now = time.time()
hari.create_media_objects(dataset_id=dataset_id, media_objects=objects)
print(f"{i}: Time to create {len(objects)} media objects: {time.time()-now}")
print(
    f"{i}: average time to create one media objects: {(time.time()-now)/len(objects)}"
)


# attrs=[]
# attr_id=uuid.uuid4()
# for media in all_media[:250]:
#     for i in range(1):
...
#
# attr_id2=uuid.uuid4()
# for media_object in all_media_objects[:250]:
#     for i in range(1):
#         attr=BulkAttributeCreate(
#             id=attr_id2,
