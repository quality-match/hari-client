...
n_objects = 3 # Number of objects per media
...
new_dataset = hari.create_dataset(name="BB_small_ds_upload", user_group="QM-ops")
...
for i in range(n_media):
    media_list.append(hari_uploader.HARIMedia(
        file_path=f"images/ecp{i+1}.png",
        name=f"camera_{i+1}",
        back_reference=f"image{i+1}",
...
for i in range(n_media):
    ...
    object3 = hari_uploader.HARIMediaObject(
        ...
        reference_data=models.PolyLine2DFlatCoordinates(
            coordinates=[450, 550, 650, 700],
            closed=False,
        ),
    )
    object3.set_object_category_subset_name("category 3")
...
uploader = hari_uploader.HARIUploader(client=hari, dataset_id=dataset_id, object_categories={"category 1", "category 2", "category 3"})
